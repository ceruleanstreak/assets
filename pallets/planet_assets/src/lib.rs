...
pub mod pallet {
	...
	pub enum Event<T: Config> {
		...
		AssetPriceSet { asset_id: u32, price: Option<BalanceOf<T>> },
		/// An asset was successfully transferred.
		AssetTransferred { from: T::AccountId, to: T::AccountId, asset_id: u32 },
		/// An asset was successfully sold.
		AssetSold { seller: T::AccountId, buyer: T::AccountId, asset_id: u32, price: BalanceOf<T> },
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn set_asset_price(
			...
			Self::deposit_event(Event::AssetPriceSet { asset_id, price: new_price });
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn mintasset(owner: &T::AccountId, asset_id: u32) -> Result<u32, DispatchError> {
			...
			let asset = Asset::<T> { asset_id, price: None, owner: owner.clone() };

			// Check if the asset does not already exist in our storage map
			ensure!(!Assets::<T>::contains_key(&asset.asset_id), Error::<T>::DuplicateAsset);
			...
			Self::deposit_event(Event::AssetCreated { asset_id, owner: owner.clone() });
		...
		pub fn do_transfer_asset(
			...
			to_owned.try_push(asset_id).map_err(|()| Error::<T>::TooManyOwned)?;
